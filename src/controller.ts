...
const verifyCaptcha = async (token: string) => {
    ...
    try {
      const response = await axios.post(
        ...
        null, // No body data, just query params
        {
          params: {
            secret: secret,
            response: token
...
export const registerUser = TryCatch(async (req, res) => {
    ...
    const token = jwt.sign({ id: user._id }, process.env.JWT_SEC as string, { expiresIn: '7d' });
...
try{if (!email || !password || !recaptchaToken ) {
...
export const forgotPassword=TryCatch(async(req,res)=>{
    ...
    const clientUrl = process.env.CLIENT_URL as string || 'http://localhost:5173'; // Default to a fallback URL if not set
...
export const updateSearchHistory = TryCatch(async (req: AuthenticatedRequest, res: Response) => {
    const { userId } = req.params;
    const { term } = req.body;
